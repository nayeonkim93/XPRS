...
option_list = list(
  make_option(c("-t", "--test.file"), type="character", default=NULL, 
              help="full path to reference.genomic.file", metavar="file"),
...
if (is.null(opt$test.file)){
  ...
  stop("An arugment must be supplied for reference.genomic.file", call.=FALSE)
}

if (is.null(opt$beta.file)){
  print_help(opt_parser)
  stop("An arugment must be supplied for PRS.scroing.file", call.=FALSE)
}
...
# 1-1. reference.genomic.file (plink file)
test.file <- opt$test.file

# 1-2. individual.genomic.file (plink file)
indi.file <- opt$indi.file
...
# 3. GWAS :gwas summary statistics that is used to construct PRS.scoring.file
...
# this function include snp match which match a1, a2 allele in reference.genomic.file and PRS.scoring.file
...
print(paste0("Time it takes to pre-processing for reference is ", time_taken_minutes, " minutes"))

# head(annotated_info)

################################# main-processing : PRS calculation for reference  #################################
# load fam file 
...
snp$original_beta <- snp$beta
snp$beta <- snp$beta * sd(full_PRS)
end_time <- Sys.time()
print(paste0("Time it takes to calculate PRS for reference is ",end_time - start_time, " seconds"))
...
# WE DON'T SAVE PRS_info
PRS_info <- as.data.frame(PRS_info)

# calculate Attributed Value of Gene and Variance using Gene Contribution Score 
annotated_info$mean <- NA
annotated_info$variance <- NA
for (i in c(1:nrow(annotated_info))){
  annotated_info$mean[i] <- mean(PRS_info[,i])
  annotated_info$variance[i] <- var(PRS_info[,i])
}
...
list_of_objects <- list(annotated_info = annotated_info, snp_info = snp, important_genes = important_genes, full_prs = FULL_PRS)
...
end_time <- Sys.time()
print(paste0("Time it takes for post-processing for reference is ",end_time - start_time, " seconds"))
end_time_T <- Sys.time()
