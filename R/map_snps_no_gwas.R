...
map_snps_no_gwas <- function(snp, beta.file, annotation.file, cS2G.file, test.file, window_size, numCores){
  ...
  #   if (nrow(snp[cS2G$SNP[i] == snp$snpid,]) != 0){
  ...
  ########## 3-2.Snp heritability-based mapping: snps mapping to gene with position and Snp heritability##########
  ...
  while (nrow(snp_not) != 0) {
    ...
    ls_2 <- c() # empty list
    max_snp <- snp_not[snp_not$h2 == max(snp_not$h2),]
    snp_not_new <- snp_not[(snp_not$chr == max_snp$chr) & (snp_not$pos > max_snp$pos - window_size) & (snp_not$pos < max_snp$pos + window_size),]
    ls_2 <- append(ls_2, snp_not_new$snpid)
    final[i,] <- NA
    final$chr[i] <- max_snp$chr
    final$start[i] <- max_snp$pos - window_size
    final$end[i] <- max_snp$pos + window_size
...
print("Done with 3-2. GWAS mapping")
